...
export default async function PaymentSuccessPage({ searchParams }: { searchParams: Promise<{ reference?: string | string[] }> }) {
  ...
  const reference = String(Array.isArray(rawReference) ? rawReference[0] : rawReference || '').split(',')[0].trim().slice(0, 160)
  ...
  try {
    ...
    return <PublicShell><Section eyebrow="Payment successful" title="Payment completed — you are enrolled"><Card><p><strong>{result.programme.name}</strong></p><p>Your course enrolment is now active. Class dates and joining information will appear in your portal when published by the programme team.</p><p className="muted">Reference: {result.reference}</p>{result.referral.attributionStatus === 'APPROVED' ? <p className="form-message success">Referral recorded successfully: {result.referral.referralCode}</p> : null}<ButtonLink href="/app" variant="primary">View My Course</ButtonLink></Card></Section></PublicShell>
  } catch (error) {
