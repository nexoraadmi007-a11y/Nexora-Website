...
export async function POST(request: NextRequest) {
  ...
  await db.from('partners').update({ user_id: user.id, whatsapp: clean('whatsapp', 40), gender: clean('gender', 40), institution: clean('institution'), field_of_study: clean('field_of_study'), nysc_information: clean('nysc_information', 300), location: clean('location'), updated_at: new Date().toISOString() }).eq('id', partner.id).throwOnError()
