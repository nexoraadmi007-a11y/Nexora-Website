...
import { findAssociateByTelegramUserId, generateSalesAssistant } from '@/lib/growth-actions'
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('Sales assistant failed', error instanceof Error ? error.message : error)
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Sales assistant failed.' }, { status: 500 })
  }
}
