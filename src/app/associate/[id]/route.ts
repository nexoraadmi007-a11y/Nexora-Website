...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!user) return NextResponse.redirect(new URL(`/growth/login?associate=${encodeURIComponent(id)}`, publicSiteUrl()))
  ...
  if (!partner || partner.status !== 'ACTIVE' || (partner.user_id && partner.user_id !== user.id) || (!partner.user_id && partner.email?.toLowerCase() !== user.email?.toLowerCase())) {
    return NextResponse.redirect(new URL('/growth/login?error=unauthorized', publicSiteUrl()))
  }
