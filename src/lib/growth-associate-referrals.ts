...
export const COMMISSION_PER_EXTRA_REFERRAL_NGN = 2000
...
export async function requireGrowthAssociateDashboard() {
  ...
  let { data: partner } = await db.from('partners').select('id,user_id,partner_id,full_name,email,whatsapp,status,referral_codes(id,code,referral_url,active)').eq('user_id', user.id).maybeSingle()
  if (!partner) {
    const lookup = await db.from('partners').select('id,user_id,partner_id,full_name,email,whatsapp,status,referral_codes(id,code,referral_url,active)').ilike('email', user.email).maybeSingle()
    partner = lookup.data
  ...
  const [performanceResult, historyResult, eventsResult, payoutsResult, leaderboardResult] = await Promise.all([
...
export async function adminGrowthReferralData(requestedMonth?: string) {
  ...
  const [partners, performance, conversions, payouts] = await Promise.all([
    db.from('partners').select('id,partner_id,full_name,email,whatsapp,status,referral_codes(code,referral_url,active)').order('full_name'),
